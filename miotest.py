...
def archiviarisultato(dnsv, tcpv, serverv, inviov):
	conn = pymysql.connect(host='127.0.0.1', port=3306, user='speeduser', passwd='speeduser', db='speedtest')
	cur = conn.cursor()
        query = "insert into speedtest.speedtestresults (dns, tcpconnection, serverprocess, contentransfer) values (%s,%s,%s,%s)" % (dnsv, tcpv, serverv, inviov)
        print(query)
	cur.execute(query)
        conn.commit()
	cur.close()
...
def main():
    ...
    if dasalvare is True:
    	archiviarisultato(d['range_dns'], d['range_connection'], d['range_server'],d['range_transfer'])
    print()
    print(stat)
